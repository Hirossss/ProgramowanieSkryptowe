...
class Animal:
    ...
    def move(self, direction: MoveDirection, validator: IMoveValidator) -> None:
        # slownik, kluczem kierunek, wartosci to akcje
        direction_actions = {
            MoveDirection.RIGHT.name: lambda: self.orientation.next(),
            MoveDirection.LEFT.name: lambda: self.orientation.previous(),
            MoveDirection.FORWARD.name: lambda: self.position.add(
                self.orientation.toUnitVector()
            ),
            MoveDirection.BACKWARD.name: lambda: self.position.subtract(
                self.orientation.toUnitVector()
            ),
        }

        # ze slownika parsujemy pozadana akcje
        ...
        if action:
            ...
            if direction.name in [
                MoveDirection.BACKWARD.name,
                MoveDirection.FORWARD.name,
            ]:
