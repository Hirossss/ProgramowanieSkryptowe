        ...
        if student in self.students:
            ...
            if self.__name in student_subjects:
                ...
                if 1 <= grade_number <= len(grades):
                    ...
                    print(
                        f"Usunięto ocenę {removed_grade} studentowi: {student._name} w przedmiocie: {self.__name}."
                    )
                else:
                    print(
                        f"Nie ma oceny o numerze {grade_number} dla studenta {student._name}."
                    )
            else:
                print(
                    f"Student {student} nie jest zapisany na przedmiot {self.__name}."
                )
        else:
